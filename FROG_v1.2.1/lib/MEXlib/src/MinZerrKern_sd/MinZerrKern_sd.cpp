...
{
	if (nrhs < 3)
		mexErrMsgTxt("Too few arguments in MINZERRKERN_sd.\n");

	if (nrhs > 3)
		mexErrMsgTxt("Too many arguments in MINZERRKERN_sd.\n");

	if (nrhs < 0)
		mexErrMsgTxt("Too few outputs in MINZERRKERN_sd.\n");

	if (nlhs > 1)
		mexErrMsgTxt("Too many outputs in MINZERRKERN_sd.\n");
 ...
 */
void MinZerrKern_sd(const TmexArray &Esig, const TmexArray &Et0, const TmexArray &dZ, TmexArray &X)
{
	int Nt		= Esig.size_M();
	...
	for(int t = 0; t < Nt; ++t)
		for(int tau = 0; tau < Ntau; ++tau) {
			...
			int tp = t - (tau - Ntau/2);
			if (tp >= 0 && tp < Nt) {
				TCmplx a	= conj(Et0[t]) *  Et0[tp] * Et0[tp] - Esig(t,tau);
				TCmplx b	= 2.0 * conj(Et0[t]) * Et0[tp] * dZ[tp] + conj(dZ[t]) * Et0[tp] * Et0[tp];
				TCmplx c	= 2.0 * conj(dZ[t]) * dZ[tp] * Et0[tp] + conj(Et0[t]) * dZ[tp] * dZ[tp];
				TCmplx d	= conj(dZ[t]) * dZ[tp] * dZ[tp];
