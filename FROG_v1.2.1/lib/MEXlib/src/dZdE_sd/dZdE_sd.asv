...
/**	Calculates the gradient of the Z error for sd.
 *
...
void dZdE_sd(const TmexArray &Esigp, const TmexArray &Et, TmexArray &dZ)
{
	...
	for(int t0 = 0; t0 < M; ++t0)
	{
		...
		for(int tau = 0; tau < N; ++tau) {
			int tp = t0 - (tau - N/2);
			if (tp >= 0 && tp < M) {
				T += ( Et[t0] * conj(Et[tp]) * conj(Et[tp]) - conj(Esigp(t0,tau)))*(Et[tp]*Et[tp]) ;				
			}
			tp = t0 + (tau - N/2);
			if (tp >= 0 && tp < M) {
				T +=  ( conj(Et[tp]) * Et[t0]*Et[t0] - Esigp(tp,tau)) * 2.* Et[tp] * conj(Et[t0]) ;
			}
		}
		dZ[t0] = (T/sz);
	}
