function [EtB, Et, EtBp, G_f, Gp_f, k] = QuickFROG_tT_mod_2(Asig, Et, t, w, options, Gpcutoff)
...
while error_cond &&  Z(end) > Zcutoff && k < n
    ...
    Gprime(k,1) = sqrt(trapz(trapz(abs(Asig.^2 - a(k)*abs(Esig).^2).^2))/trapz(trapz(Asig.^4)));
    ...
    if k > 50
        ...
        if (stag < 5e-8 && stag < Gcutoff/1000); break; end
    end
