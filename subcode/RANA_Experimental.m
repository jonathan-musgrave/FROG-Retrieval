function [Et_final, Gout, Gpout, Sout] = RANA_Experimental(I_FROG, time, G_cut, Gprime_cut,npad) 
...
if isempty(npad)
    ...
    freq = 1/npad/dt*h; % SHG Frog padded and cropped to the correct inx 
end
% Crop = h+npad/2+1; % Array for cropping every axis
...
Swr{1} = spec_ret_marginal(Mw, 0, 0,npad); % Mw: frequency marginal, 0/1 : noise, 0/1: 4 spectra/ 1 spectrum 
    % BD
    % Swr{1} = (Swr{1}'.*exp(-(freq'./1e12).^8))';
    
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ %
% GENERATING BINNED ARRAYS FOR EACH GRID

[Isig_wT, t, f, Swr] =   multi_grid(Isig_wT{1}, t{1}, f{1}, Swr{1},npad) ;
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ %
...
for jj =  tot_initial(k):-num_spec:1
   if num_spec == 1 
        ...
        % et03(jj-num_spec+1:jj,:) = temp(Crop);
   end
end
...
for i_ret =  k :-1:1
	...
	elseif i_ret == 3   % N/4 by N/4 grid
                ...
                for i_initial = 1 :tot_initial(i_ret)
	...
	elseif i_ret == 1 % N by N grid
                ...
                Gpout = Gprime_error(shg_amp, Et_retr(:,ind1(1)),npad);
                ...
                elseif  Gp_sure <= Gprime_cut
                    ...
                    Gpout = Gp_sure;
                    G_out_dum = G_error(shg_amp, Et_final,npad);
                ...
                if condition ~= 1
                    ...
                    for i_initial = 1: tot_initial(1)
                        ...
                        if G_f(i_initial) < G_out_dum
                            ...
                            Gpout = Gprime_error(shg_amp, Et_final,npad);
...
Gout = G_error(abs(shg_amp), Et_final,npad);
Gpout = Gprime_error(abs(shg_amp), Et_final,npad);
